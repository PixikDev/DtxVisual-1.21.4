package pixikdev.ru.dtxvisual.modules.api;
...
import pixikdev.ru.dtxvisual.client.util.notify.NotifyIcons;
...
public abstract class Module implements Wrapper {
    ...
    public void onEnable() {
        ...
        if (!fullNullCheck() && !name.equals("UI")) {
            ...
            DtxVisual.getInstance().getNotifyManager().add(new Notify(NotifyIcons.successIcon, msg, 1000));
        }
    }

    public void onDisable() {
        toggled = false;
        DtxVisual.getInstance().getEventHandler().unsubscribe(this);
        if (!fullNullCheck() && !name.equals("UI")) {
            String translatedName = I18n.translate(name);
            String msg = I18n.translate("notify.featureDisabled", translatedName);
            DtxVisual.getInstance().getNotifyManager().add(new Notify(NotifyIcons.failIcon, msg, 1000));
        }
    }
