package pixikdev.ru.dtxvisual.client.ui.clickgui;

import pixikdev.ru.dtxvisual.DtxVisual;
import pixikdev.ru.dtxvisual.client.ui.clickgui.components.impl.ModuleComponent;
...
import pixikdev.ru.dtxvisual.client.managers.ThemeManager; // Import ThemeManager
import pixikdev.ru.dtxvisual.modules.api.Category;
import pixikdev.ru.dtxvisual.modules.api.Module;
import pixikdev.ru.dtxvisual.modules.impl.render.UI;
import pixikdev.ru.dtxvisual.modules.impl.utility.ClientSound;
...
public class ClickGui extends Screen implements Wrapper {
    ...
    public ClickGui() {
        super(Text.of("dtxvisual-clickgui"));
    ...
    private void buildComponentsCache() {
        ...
        for (Category cat : TABS) {
            if (cat != Category.Theme) { // Skip Theme category for module components
                List<Module> mods = DtxVisual.getInstance().getModuleManager().getModules(cat);
    ...
    private void playToggleSound(boolean wasToggled) {
        ClientSound clientSound = DtxVisual.getInstance().getModuleManager().getModule(ClientSound.class);
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (closing) {
            ...
            if (yAnimation.getValue() <= 0.01f) {
                DtxVisual.getInstance().getModuleManager().getModule(UI.class).setToggled(false);
                super.close();
                return;
            }
        } else {
    ...
    private void renderBottomHints(DrawContext ctx) {
        ...
        String hint1 = I18n.translate("dtxvisual.clickgui.hint.bind");
        String hint2 = I18n.translate("dtxvisual.clickgui.hint.settings");
        String hint3 = I18n.translate("dtxvisual.clickgui.hint.hud_move");
